...
After login, every `POST` / `PUT` / `DELETE` (except the login POST itself) must include header `X-CSRF-Token` matching `csrf_token` from `GET /api/auth.php`. The public vendor quote form does not send a token.
